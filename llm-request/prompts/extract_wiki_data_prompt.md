...
3. University city
4. University state
5. Primary membership period in the conference (start year, end year if known)
...
5. City and state should represent the university's campus location in current/common usage when present in source text.
6. Keep years as integers.
7. Use `null` for unknown `end_year` (ongoing membership) or unavailable values.
8. If multiple universities have similar names, `university_wikipedia_href` is the primary disambiguator.
